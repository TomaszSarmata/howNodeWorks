...
server.on("request", (req, res) => {
  ...
  //   });
  // Solution 3 - that will sort out the back pressure that we were getting with the solution 2

  const readable = fs.createReadStream("test-file.txt");
  readable.pipe(res); //here we are simply piping the response
  // so the rule is: readableSource.pipe(wiriteableDestination)
});
